    ...
    {setup,
        ...
        [{"Handle ping query. Querying node is in the bucket.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, get_not_assigned_node, ['_', {12,34,92,155}, 6862])
                )
            end
        }]
    }.
    ...
    {setup,
        fun() ->
            ...
            ok = meck:expect(erline_dht_db_ets, get_info_hash, fun
                (_, {12,34,92,153}, 6860, <<0,5>>) -> false;
                (_, {12,34,92,154}, 6861, <<0,5>>) -> <<"1nf0_h45h">>
            end),
            ok = meck:expect(erline_dht_db_ets, get_requested_node, fun
                (_, {12,34,92,156}, 6863, <<"1nf0_h45h">>) -> [];
                (_, {12,34,92,157}, 6864, <<"1nf0_h45h">>) -> [#requested_node{ip_port = {{12,34,92,157}, 6864}}];
                (_, {12,34,92,158}, 6865, <<"1nf0_h45h">>) -> []
            end),
            ok = meck:expect(erline_dht_db_ets, insert_to_get_peers_searches, ['_', #get_peers_search{info_hash = <<"1nf0_h45h">>, last_changed = {{2020,7,1},{12,0,0}}}], true)
        end,
        fun(_) ->
            true = meck:validate([erline_dht_message, erline_dht_helper, erline_dht_db_ets]),
            ok = meck:unload([erline_dht_message, erline_dht_helper, erline_dht_db_ets])
        ...
        {"Handle get_peers response. Info hash is found. Got nodes.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, insert_to_get_peers_searches, ['_', #get_peers_search{info_hash = <<"1nf0_h45h">>, last_changed = {{2020,7,1},{12,0,0}}}])
                ),
                ?assertEqual(
                    3,
                    meck:num_calls(erline_dht_db_ets, get_requested_node, ['_', '_', '_', '_'])
                ),
                ?assertEqual(
                    1,
...
%%
%%
handle_announce_peer_query_test_() ->
    EventMgrPid = erlang:list_to_pid("<0.0.100>"),
    State = #state{
        event_mgr_pid = EventMgrPid,
        db_mod        = erline_dht_db_ets,
    ...
    {setup,
        fun() ->
            ok = meck:new([erline_dht_message, erline_dht_helper, erline_dht_db_ets]),
            ok = meck:expect(erline_dht_message, respond_announce_peer, [{12,34,92,155}, 6862, sock, <<0,2>>, <<"h45h">>], ok),
            ok = meck:expect(erline_dht_message, respond_error, [{12,34,92,155}, 6862, sock, <<0,2>>, 203, <<"Bad Token">>], ok),
            ok = meck:expect(erline_dht_helper, notify, fun
                (MatchEventMgrPid, {announce_peer, q, {12,34,92,155}, 6862, {<<"n0d3_h45h">>, <<"1nf0_h45h00">>, 6861, <<"t0k3n">>}}) when MatchEventMgrPid =:= EventMgrPid -> ok;
                (MatchEventMgrPid, {announce_peer, q, {12,34,92,155}, 6862, {<<"n0d3_h45h">>, <<"1nf0_h45h00">>, 6862, <<"t0k3n">>}}) when MatchEventMgrPid =:= EventMgrPid -> ok;
                (MatchEventMgrPid, {announce_peer, q, {12,34,92,155}, 6862, {<<"n0d3_h45h">>, <<"1nf0_h45h00">>, 6862, <<"n0t_3x15t1ng_t0k3n">>}}) when MatchEventMgrPid =:= EventMgrPid -> ok
            end),
            ok = meck:expect(erline_dht_helper, local_time, [], {{2020,7,1},{12,0,0}}),
            ok = meck:expect(erline_dht_db_ets, get_not_assigned_node, ['_', {12,34,92,155}, 6862], [#node{ip_port = {{12,34,92,155}, 6862}, hash = <<"h45h_self">>, token_sent = <<"t0k3n">>}]),
            ok = meck:expect(erline_dht_db_ets, update_not_assigned_node, ['_', #node{ip_port = {{12,34,92,155}, 6862}, hash = <<"h45h_self">>, token_sent = <<"t0k3n">>, last_changed = {{2020,7,1},{12,0,0}}}], true)
        end,
        fun(_) ->
            true = meck:validate([erline_dht_message, erline_dht_helper, erline_dht_db_ets]),
        ...
        [{"Handle announce_peer query. Implied port is 0. Sent token is found. ",
            fun() ->
                ?assertEqual(
                    State#state{info_hashes = [#info_hash{info_hash = <<"1nf0_h45h00">>, peers = [{{12,34,92,155},6861}]}]},
                    erline_dht_bucket:handle_announce_peer_query({12,34,92,155}, 6862, <<"n0d3_h45h">>, 0, <<"1nf0_h45h00">>, 6861, <<"t0k3n">>, <<0,2>>, State)
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_message, respond_announce_peer, [{12,34,92,155}, 6862, sock, <<0,2>>, <<"h45h">>])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, notify, [EventMgrPid, {announce_peer, q, {12,34,92,155}, 6862, {<<"n0d3_h45h">>, <<"1nf0_h45h00">>, 6861, <<"t0k3n">>}}])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, local_time, [])
                ...
                ?assertEqual(
                    0,
                    meck:num_calls(erline_dht_message, respond_error, ['_', '_', '_', '_', '_', '_'])
                ),
                ok = meck:reset([erline_dht_message, erline_dht_helper, erline_dht_db_ets])
            end
        },
        {"Handle announce_peer query. Implied port is 1. Sent token is found. ",
            fun() ->
                ?assertEqual(
                    State#state{info_hashes = [#info_hash{info_hash = <<"1nf0_h45h00">>, peers = [{{12,34,92,155},6862}]}]},
                    erline_dht_bucket:handle_announce_peer_query({12,34,92,155}, 6862, <<"n0d3_h45h">>, 1, <<"1nf0_h45h00">>, 6861, <<"t0k3n">>, <<0,2>>, State)
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_message, respond_announce_peer, [{12,34,92,155}, 6862, sock, <<0,2>>, <<"h45h">>])
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, notify, [EventMgrPid, {announce_peer, q, {12,34,92,155}, 6862, {<<"n0d3_h45h">>, <<"1nf0_h45h00">>, 6862, <<"t0k3n">>}}])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, local_time, [])
        ...
        {"Handle announce_peer query. Sent token is not found. ",
            fun() ->
                ?assertEqual(
                    State,
                    erline_dht_bucket:handle_announce_peer_query({12,34,92,155}, 6862, <<"n0d3_h45h">>, 1, <<"1nf0_h45h00">>, 6861, <<"n0t_3x15t1ng_t0k3n">>, <<0,2>>, State)
                ),
                ?assertEqual(
                    0,
                    meck:num_calls(erline_dht_message, respond_announce_peer, ['_', '_', '_', '_', '_'])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, notify, [EventMgrPid, {announce_peer, q, {12,34,92,155}, 6862, {<<"n0d3_h45h">>, <<"1nf0_h45h00">>, 6862, <<"n0t_3x15t1ng_t0k3n">>}}])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, local_time, [])
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_message, respond_error, [{12,34,92,155}, 6862, sock, <<0,2>>, 203, <<"Bad Token">>])
                ),
                ok = meck:reset([erline_dht_message, erline_dht_helper, erline_dht_db_ets])
            end
        }]
    ...
    {setup,
        ...
        {"Generic handler. Handle response. Bucket is not found. Bucket has some free space. Do not need to do find_nodes.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, delete_from_not_assigned_nodes_by_ip_port, ['_', {12,34,92,185}, 6882])
                ),
                ?assertEqual(
                    0,
...
get_bucket_and_node_test_() ->
    ...
    {setup,
        fun() ->
            ...
            ok = meck:expect(erline_dht_db_ets, get_not_assigned_node, fun
                (_, {12,34,92,156}, 6863) -> [];
                (_, {12,34,92,157}, 6864) -> [#node{ip_port = {{12,34,92,157}, 6864}}];
                (_, {12,34,92,158}, 6865) -> []
            end)
        end,
        fun(_) ->
    ...
    {setup,
        ...
        {"Update node in a not assigned nodes list and assign to a bucket.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, delete_from_not_assigned_nodes_by_ip_port, ['_', {12,34,92,154}, 6861])
                ),
                ok = meck:reset(erline_dht_db_ets)
            end
        },
    ...
    {setup,
        ...
        [{"Bucket is not full.",
            fun() ->
                ...
                ?assertEqual(
                    0,
                    meck:num_calls(erline_dht_db_ets, get_not_assigned_node, ['_', '_', '_'])
                ),
                ?assertEqual(
                    0,
                    meck:num_calls(erline_dht_db_ets, update_not_assigned_node, ['_', '_'])
        ...
        {"Bucket is full. Not active node is moved to not assigned nodes list.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, get_not_assigned_node, ['_', {12,34,92,156}, 6863])
                ),
                ?assertEqual(
                    1,
...
insert_info_hash_test_() ->
    ...
    {setup,
        fun() ->
            ...
            ok = meck:expect(erline_dht_db_ets, get_not_assigned_node, fun
                (_, {12,34,92,156}, 6863) -> [#node{ip_port = {{12,34,92,156}, 6863}, hash = <<"h45h1">>}];
                (_, {12,34,92,157}, 6864) -> [#node{ip_port = {{12,34,92,157}, 6864}, hash = <<"h45h2">>}];
                (_, {12,34,92,158}, 6865) -> [#node{ip_port = {{12,34,92,158}, 6865}, hash = <<"h45h3">>}];
        ...
        [{"Requested nodes not found.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, get_requested_nodes, ['_', <<"1nf0_h45h1">>])
                ),
                ?assertEqual(
                    0,
        ...
        {"Requested nodes found.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, get_requested_nodes, ['_', <<"1nf0_h45h2">>])
                ),
                ?assertEqual(
                    4,
                    meck:num_calls(erline_dht_helper, get_distance, ['_', '_'])
...
check_searches_test_() ->
    ...
    {setup,
        fun() ->
            ...
            ok = meck:expect(erline_dht_db_ets, get_all_get_peers_searches, ['_'], [
            ...
            ok = meck:expect(erline_dht_db_ets, get_requested_nodes, ['_', '_'], []),
            ok = meck:expect(erline_dht_db_ets, delete_get_peers_search, ['_', '_'], true),
            ok = meck:expect(erline_dht_db_ets, delete_requested_nodes, ['_', '_'], true),
        ...
        [{"After search is exhausted, insert infohash with contact information of itself and clear old peers searches.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, get_all_get_peers_searches, ['_'])
                ),
                ?assertEqual(
                    2,
                    meck:num_calls(erline_dht_db_ets, delete_get_peers_search, ['_', '_'])
                ),
                ?assertEqual(
                    2,
                    meck:num_calls(erline_dht_db_ets, delete_requested_nodes, ['_', '_'])
                ),
                ?assertEqual(
                    3,
                    meck:num_calls(erline_dht_helper, local_time, [])
    ...
    {setup,
        ...
        [{"Initiate not actives nodes replacement.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, get_not_assigned_nodes, ['_', '_'])
                ),
                ?assertEqual(
                    10,
                    meck:num_calls(erline_dht_db_ets, update_not_assigned_node, ['_', '_'])
...
clear_not_assigned_nodes_test_() ->
    ...
    {setup,
        fun() ->
            ok = meck:new([erline_dht_db_ets, erline_dht_helper]),
            ok = meck:expect(erline_dht_db_ets, get_not_assigned_nodes, ['_', 0], ?NODES_LIST),
            ok = meck:expect(erline_dht_db_ets, delete_from_not_assigned_nodes_by_ip_port, ['_', '_', '_'], true),
            ok = meck:expect(erline_dht_db_ets, delete_from_not_assigned_nodes_by_dist_date, ['_', undefined, {{2020,7,1},{12,0,0}}], ok),
        ...
        [{"Removable nodes are the same amount as threshold",
            fun() ->
                ...
                ?assertEqual(
                    3,
                    meck:num_calls(erline_dht_db_ets, delete_from_not_assigned_nodes_by_ip_port, ['_', '_', '_'])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, local_time, [])
        ...
        {"Clear nodes without distance only.",
            fun() ->
                ...
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_db_ets, delete_from_not_assigned_nodes_by_dist_date, ['_', undefined, {{2020,7,1},{12,0,0}}])
                ),
                ?assertEqual(
                    1,
                    meck:num_calls(erline_dht_helper, local_time, [])
